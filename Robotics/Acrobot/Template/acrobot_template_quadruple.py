...
def train(run, hyper_parameters, variables):
    ...
    q_table_b = np.zeros(num_buckets + (num_actions,)) if q_table_settings[0] == 0 else np.random.randn(
        *num_buckets, num_actions) * q_table_settings[0] + q_table_settings[1]
    q_table_c = np.zeros(num_buckets + (num_actions,)) if q_table_settings[0] == 0 else np.random.randn(
        *num_buckets, num_actions) * q_table_settings[0] + q_table_settings[1]
    q_table_d = np.zeros(num_buckets + (num_actions,)) if q_table_settings[0] == 0 else np.random.randn(
        *num_buckets, num_actions) * q_table_settings[0] + q_table_settings[1]
...
def update_q_table(q_table_a, q_table_b, q_table_c, q_table_d, state_information, parameters, termination):
    ...
    if random_chance < 0.25:
        ...
        q_table_c[state_0 + (opposite_action,)] += opposite_penalty
        ...
        q_table_d[state_0 + (opposite_action,)] += opposite_penalty
