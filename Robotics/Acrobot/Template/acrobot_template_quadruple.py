...
def train(run, variables):
    ...
    q_table_a = np.random.randn(*num_buckets, num_actions) * q_table_type
    q_table_b = np.random.randn(*num_buckets, num_actions) * q_table_type
    q_table_c = np.random.randn(*num_buckets, num_actions) * q_table_type
    q_table_d = np.random.randn(*num_buckets, num_actions) * q_table_type
    ...
    for episode in range(1, NUM_TRAIN_EPISODES + 1):
        ...
        for t in range(1, MAX_TRAIN_T + 1):
            ...
            # Select an action
            action = select_action(state_0, explore_rate, q_table_a + q_table_b + q_table_c + q_table_d, num_actions)
            ...
            if terminated:
                ...
                parameters = (learning_rate, opposite_penalty)
                termination = (terminated, termination_penalty)
                update_q_table(q_table_a, q_table_b, q_table_c, q_table_d, state_information, parameters, termination)
...
def update_q_table(q_table_a, q_table_b, q_table_c, q_table_d, state_information, parameters, termination):
    state_0, state, action, reward = state_information
    learning_rate, opposite_penalty = parameters
    terminated, termination_penalty = termination

    opposite_action = (action + 1) % 2
    reward = termination_penalty if terminated else reward

    q_table_all = [q_table_a, q_table_b, q_table_c, q_table_d]
    q_table_main = q_table_all.pop(random.randint(0, 3))
    q_table_secondary = q_table_all.pop(random.randint(0, 2))
...
def run_simulation(seed_random, run_settings, variables):
    random_seed(seed_random)
    results, failed = loop(run_settings, variables)
